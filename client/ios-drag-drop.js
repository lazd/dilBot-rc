(function(doc) {
  ...
  DragDrop.prototype = {
    ...
    move: function(event) {
      ...
      Array.prototype.forEach.call(event.changedTouches,function(touch, index) {
    ...
    dispatchDrag: function(event) {
      var target = elementFromTouchEvent(this.el,event);

      if (!target) return;

      var dragEvt = doc.createEvent("Event");
      dragEvt.initEvent("dragover", true, true);

      // A little hack to get values to match desktop
      dragEvt.layerX = target.offsetWidth/2 + event.layerX - event.target.offsetWidth/2;
      dragEvt.layerY = target.offsetHeight/2 + event.layerY - event.target.offsetHeight/2;
